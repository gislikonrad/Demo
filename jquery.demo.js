(function($, window, undefined) {
	...
	// TODO: refactor for support for multiple elements at once
	$.extend($.fn, {
		demo: function(options) {
			var self = this,
				...
				parent = self.parent(),
				source = self[0].outerHTML,
				parentSource = parent[0].outerHTML,
				parentIndex = parentSource.indexOf(source),
			...
			for(var i = 1; i < parentIndex; i++) {
				startIndex = parentIndex - i;
				character = parentSource.charAt(startIndex);
				if(!indentRegex.test(character)) {
					break;
				}
			}
			
			source = parentSource.substr(startIndex, source.length + parentIndex - startIndex);
			lines = source.split(newLine);
			lineNumberLength = lines.length.toString().length;
			
			for(var i = 0; i < lines.length; i++){
				if(firstIndentLength === undefined) {
					match = indentRegex.exec(lines[i]);
					firstIndentLength = match[0].length;
				}
				lines[i] = lines[i].substr(firstIndentLength).replace(tabRegex, o.indent);
				if(o.lineNumbers === true){
					var number = ( i + 1 ).toString();
					while(number.length < lineNumberLength){
						number = '0' + number;
					}
			...
			hljs.highlightBlock(pre.children('code').text(source)[0]);
			return pre;
		}
	});
})(jQuery, this);